...
while ($leaderboard_row = $leaderboard_user_query_result->fetch_assoc()) {
    ...
    $score_row = $leaderboard_score_query_result->fetch_assoc();
        ...
        for ($i = 0; $i <= D_total_number_of_questions; $i++) {
            $leaderboard_question_query_result = $mysqli->query("SELECT TIMESTAMPDIFF(SECOND,(SELECT start_time FROM constants LIMIT 1),submit_timestamp) as time FROM leaderboard WHERE question_number = '$i+1' AND user='$user'");
